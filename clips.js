...
function markClipClear(){
    ...
    META.projects.items[META.projects.cur_id].clip_order.forEach((item, i) => {
...
function markClipSave(){
    ...
    if (id === null) {
        ...
        //clip_order.push(id)
        META.projects.items[META.projects.cur_id].clip_order.push(id)
    }
    //let data = {}
    if (!META.projects.items[META.projects.cur_id].clips[id]) {
        META.projects.items[META.projects.cur_id].clips[id] = {}
    }
    META.projects.items[META.projects.cur_id].clips[id].id = id
    META.projects.items[META.projects.cur_id].clips[id].video_path = STATE.video_path
    META.projects.items[META.projects.cur_id].clips[id].video_filename = STATE.video_filename
    META.projects.items[META.projects.cur_id].clips[id].start = start.toFixed(3)
    META.projects.items[META.projects.cur_id].clips[id].end = end.toFixed(3)
    META.projects.items[META.projects.cur_id].clips[id].runtime = parseFloat(end - start).toFixed(3)
    ...
    function finishSave(){
        ...
        META.projects.items[META.projects.cur_id].clips[id].thumb = canvas.toDataURL();
        parseClipList()
        //seekClipStart()
        markClipClear()
        vplayer.currentTime = end
        // *** move to a seperte action *** save the clip to meta for this file
        //META.items[STATE.video_filename].clips[id] = cloneObj(META.projects.items[META.projects.cur_id].clips[id])
        saveMeta()
...
function parseClipList(){
    let str = ""
    META.projects.items[META.projects.cur_id].clip_order.forEach((item, i) => {
        let data = META.projects.items[META.projects.cur_id].clips[item]
...
function loadPrevClip(id) {
    ...
    let thisclip = META.projects.items[META.projects.cur_id].clips[id]
    vplayerPause()
